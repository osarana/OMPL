...
namespace ompl
{
    namespace app
    {
        ...
        /// @brief Wrapper for ompl::app::RigidBodyPlanning that plans for multiple rigid bodies in R3SO2.
        class R3SO2MultiRigidBodyPlanning : public AppBase<AppType::GEOMETRIC>
        {
        public:
            /// @brief Constructs an instance of multiple rigid bodies for 4D geometric planning.  n is the number of independent bodies in R^3 x SO(2)
            R3SO2MultiRigidBodyPlanning(unsigned int n);

            ~R3SO2MultiRigidBodyPlanning() override = default;
